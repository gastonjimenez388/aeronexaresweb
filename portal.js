...
function requireRole(role){ const user=JSON.parse(sessionStorage.getItem('portalUser')||'null'); if(!user||user.role!==role){ location.href='portal-login.html?v=secure6'; return null; } document.querySelectorAll('[data-user]').forEach(e=>e.textContent=user.id); return user; }
function logout(){ sessionStorage.removeItem('portalUser'); location.href='portal-login.html?v=secure6'; }
